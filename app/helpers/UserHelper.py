...
class UserHelper:
    ...
    def update_authorization_status(
        cls, user_telegram_id: int, is_authenticated: bool
    ) -> None:
        user = cls.__get_user_by_telegram_id(user_telegram_id=user_telegram_id)
    ...
    @classmethod
    def reset_notification_settings(cls, user_telegram_id: int) -> None:
    ...
    @classmethod
    async def increment_failed_request_count(cls, user_telegram_id: int) -> None:
        user = cls.__get_user_by_telegram_id(user_telegram_id=user_telegram_id)
        user.failed_request_count += 1
        if user.failed_request_count > config.ORIOKS_MAX_FAILED_REQUESTS:
            logging.info(
                "Деавторизация пользователя %s, поскольку превышен лимит "
                "неудачных запросов",
                user_telegram_id,
            )
    ...
    @classmethod
    def reset_failed_request_count(cls, user_telegram_id: int) -> None:
        user = cls.__get_user_by_telegram_id(user_telegram_id=user_telegram_id)
